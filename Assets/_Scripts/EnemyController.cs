...
public class EnemyController : MonoBehaviour
{
        ...
        if(theBody.isVisible && PlayerController.instance.gameObject.activeInHierarchy)
        {
            ...
            else 
            {
                ...
                if (shouldPatrol)
                {
                    ...
                    if (Vector3.Distance(patrolPoints[currentPatrolPoint].position,transform.position)<.2f)
                    {
                        currentPatrolPoint++;
            ...
            if (shouldShoot&&Vector3.Distance(transform.position,PlayerController.instance.transform.position)<=shootRange)
            {
                ...
                if (fireCounter <= 0)
                {
                    ...
                    AudioManager.instance.PlaySFX(AudioManager.AudioCode.Shoot2);
                }
            }
        }
        else
    ...
    public void DamageEnemy(int damage)
    {
        ...
        AudioManager.instance.PlaySFX(AudioManager.AudioCode.EnemyHurt);

        Instantiate(hitEffect, transform.position, transform.rotation);

        if (health <= 0)
        {
            AudioManager.instance.PlaySFX(AudioManager.AudioCode.EnemyDeath);
